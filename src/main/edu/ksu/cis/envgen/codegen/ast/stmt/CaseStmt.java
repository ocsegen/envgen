...
public class CaseStmt extends JavaStmt {
	// TODO: eliminate?
	...
	public CaseStmt(JavaExpr value, JavaStmt body, JavaStmt next) {
		this.value = value;
		this.body = body;
		this.next = next;
	}
	...
	public void printToFile(FileWriter file, JavaPrinter printer, int shift) {
		...
		if (value != null) {
			...
			// printf(file, "default");
			printf(file, ":\n");
			body.printToFile(file, printer, shift + 3);
		}
		if (next != null)
			next.printToFile(file, printer, shift);
	}
}
